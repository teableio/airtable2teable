...
export class AirtableCountField extends AirtableFieldVo {
    ...
    transformTeableCreateFieldRo(tables, newTables) {
        ...
        const mappingLinkField = newTable.fields.find((field) => linkField.name === field.name);
        const linkFieldOptions = linkField.options;
        ...
        const lookupField = foreignTable.fields.find((field) => field.id === lookupFieldId);
        const mappingForeignTable = mappingTable(tables, newTables, foreignTableId);
